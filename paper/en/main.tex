...
\title{An Auxiliary-Variable-Free Quantum Annealing
QUBO Formulation for Strongly Connected Orientation
of Large-Scale Circulation Networks}
...
\abstract
Assigning a one-way direction to every edge of an undirected network while preserving strong connectivity—reachability between every pair of locations—is a hard combinatorial optimization problem in which the number of possible direction assignments grows exponentially with the number of edges. In particular, strong connectivity is a nonlocal constraint that depends on global circulation structure spanning the entire network, and is therefore difficult to express as a penalty term of a Quadratic Unconstrained Binary Optimization (QUBO) model. This study proposes a framework that solves this problem with quantum annealing. In a preprocessing stage, a planar network is partitioned along its faces and the cluster boundaries are pre-oriented into directed cycles, erecting a global circulation skeleton that reduces the strong connectivity constraint to a local condition on each subgraph. The remaining orientation optimization uses an $n$-hop reachability surrogate objective, and the energy function stays quadratic without auxiliary variables owing to the property that directed walks pair with their reversals so that the highest-degree terms cancel. The final QUBO model is solved on the D-Wave quantum annealing platform. Experimental results show that the proposed method achieves strong connectivity in all completed trials, yields better solutions than classical constructive methods in terms of the sum of all-pairs shortest paths (APSP), and requires roughly one tenth of the computation time of classical local search, thereby providing a practical trade-off between solution quality and computation time.
\endabstract

\section{Introduction} \label{sec:intro}

The problem of assigning a traffic direction to each edge of an undirected network, known as the graph orientation problem, is a classical topic in graph theory. Robbins showed that every bridgeless connected graph admits an orientation that makes every path one-way while preserving reachability between any two locations, that is, strong connectivity~\cite{robbins1939theorem}. The problem has since evolved into the design of one-way road networks that minimize the resulting increase in travel distance~\cite{drezner1997selecting}.

Finding a good orientation, however, is computationally hard. The number of possible direction assignments grows exponentially with the number of edges, and the complexity increases further when the strong connectivity constraint and travel distance objectives must be considered simultaneously~\cite{burkard1999minimum, bangjensen2023complexity}. In particular, strong connectivity depends not on local choices of individual edges but on circulation structure spanning the entire network, making it a global constraint that is difficult to capture with low-degree local interactions. Consequently, deriving high-quality solutions for large networks within practical time using classical computing resources alone remains challenging.

This study applies quantum annealing to this problem. We abstract the target network into a weighted graph and, building upon the foundational framework of quantum annealing~\cite{Kadowaki1998}, directly formulate the orientation optimization problem as a Quadratic Unconstrained Binary Optimization (QUBO) model. The strong connectivity constraint is handled not by a penalty term but by a global circulation skeleton erected through face-based partitioning and boundary cycle pre-orientation, while the energy function encodes only short directed-path reachability and thus stays quadratic without auxiliary variables.
...
\begin{itemize}
    ...
    \item[(1)] Proposes a preprocessing scheme that erects a circulation skeleton through face-based partitioning and boundary cycle pre-orientation, handling the global circulation constraint of strong connectivity without penalty terms.
    \item[(2)] Proposes a novel auxiliary-variable-free QUBO formulation to solve the remaining orientation optimization via quantum annealing.
    \item[(3)] Demonstrates, through comparisons with classical algorithms, that the proposed method provides a practical trade-off between solution quality and computation time.
\end{itemize}
The proposed formulation is not tied to a specific application and can be applied to circulation network design problems such as one-way road network design and one-way operation of pedestrian circulation routes.
...
One-way street design has also been studied as a transportation network optimization problem. Existing approaches typically aim to reduce travel time, improve traffic capacity, or minimize the cost of modifying a road network~\cite{drezner1997selecting, huang2020bi, huang2020model}. In transportation and evacuation planning, contraflow strategies have also been studied to improve the utilization of road capacity by reversing selected traffic directions during emergencies~\cite{kim2008contraflow, aksu2014mathematical}. Nevertheless, these problems become computationally challenging as the network size increases because the number of possible direction assignments grows exponentially with the number of edges. The complexity becomes even greater when strong connectivity, arc reversal, and multiple optimization objectives must be considered simultaneously~\cite{burkard1999minimum, bangjensen2023complexity}.

Unlike these previous studies, this study handles the strong connectivity constraint through cycle-skeleton preprocessing rather than explicit constraints or penalty terms, and keeps the short-path reachability objective quadratic without auxiliary variables so that it can be placed directly on quantum annealing hardware.
...
Building on these studies, we formulate the network orientation problem as a QUBO model and evaluate its applicability on the D-Wave quantum annealing platform.

\section{System Model and Problem Formulation} \label{sec:model}

\subsection{Network Model and Decision Variables} \label{subsec:model_graph}

We consider a network represented by a weighted undirected graph $G=(V,E,W)$, where \(V\) denotes the set of intersections or junctions, and \(E\) denotes the set of traversable segments connecting them. Each undirected edge \(\{i,j\}\in E\) represents a physical passage that originally allows movement in both directions. A graph containing a bridge admits no strongly connected orientation at all~\cite{robbins1939theorem}. We therefore restrict the scope to networks for which a planar, bridgeless approximation is valid, and assume that \(G\) is planar, connected, and bridgeless. Every block is then biconnected, so the face structure is well defined. In the experiments we use planar graphs generated by Delaunay triangulation.

The weight \(w_{ij}>0\) of each edge denotes the traversal capacity of the segment. The traversal cost of the segment, that is, its distance, is accordingly defined as the reciprocal
\begin{equation}
    \ell_{ij}=\frac{1}{w_{ij}}
\end{equation}
so that higher-capacity segments are cheaper to traverse. The objective of this study is to transform the original bidirectional network into a directed one-way circulation network by assigning a single traversal direction to each segment.

For each undirected edge \(\{i,j\}\in E\), an arbitrary but fixed ordering of its endpoints is imposed such that \(i<j\), and a binary orientation variable \(x_{ij}\) is defined so that \(x_{ij}=0\) orients the edge as \(i\rightarrow j\) and \(x_{ij}=1\) orients it as \(j\rightarrow i\). The complete variable vector \(\mathbf{X}\in\{0,1\}^{|E|}\) fully determines the directed graph \(\vec{G}(\mathbf{X})=(V,\vec{E}(\mathbf{X}))\), and each physical segment is assigned exactly one direction. For notational brevity the definitions below index variables by endpoint pairs, but series contraction (Section~\ref{subsec:method_pre}) can create more than one edge joining the same pair of endpoints. In that case variables and indicator functions are distinguished by edge identifier, and every sum over \(\mathrm{adj}(v)\) is read as a sum over the edges incident to \(v\).
...
Combining the above, the network orientation problem is formulated as a binary constrained optimization problem. The only decision variable is the binary orientation vector \(\mathbf{X}\), and with the exact objective the problem reads
\begin{equation}
...
The second limitation conversely suggests an opportunity for reuse. Since minor embedding depends only on the structure of the interaction graph and not on the QUBO coefficient values, in a setting where only edge weights change while the graph structure stays fixed, the partition and the embedding can be cached and reused, with only the coefficients updated before re-sampling. This is a natural extension of the fixed-embedding reuse already employed in our experiments, and it would remove the 4{,}243 seconds of embedding search from the 4{,}260-second total at \(|V|=500\), reducing re-optimization after a weight change to the tens-of-seconds scale of QUBO solving. Our experiments, however, covered only unit weights, and the default-policy chain strength varies with coefficient magnitudes, so whether the cached partition and embedding preserve solution quality under weight changes remains future work requiring validation.
